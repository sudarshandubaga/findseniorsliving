...
@section('content')
    <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" id="post-form">
        ...
        <input type="hidden" name="tag_names" id="tag-names-input" value="{{ $post->tags->pluck('name')->implode(',') }}">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    <div class="space-y-2 max-h-48 overflow-y-auto">
                        @php $postCatIds = old('categories', $post->categories->pluck('id')->toArray()); @endphp
                        @foreach($categories as $category)
                            <label class="flex items-center gap-2 cursor-pointer hover:bg-gray-50 px-2 py-1.5 rounded-lg transition-colors">
                                <input type="checkbox" name="categories[]" value="{{ $category->id }}"
                                    ...
                                    {{ in_array($category->id, $postCatIds) ? 'checked' : '' }}>
...
<script>
    ...
    function tagInput() {
        return {
            selectedTags: {!! json_encode($post->tags->pluck('name')->toArray()) !!},
            query: '',
            suggestions: [],
            debounceTimer: null,
    ...
    function imageCropper() {
        return {
            showCropper: false,
            croppedPreview: @json($post->featured_image ? asset('storage/' . $post->featured_image) : null),
            cropper: null,
