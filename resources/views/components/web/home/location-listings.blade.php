    ...
    <div class="container">
        ...
        <div class="mb-20">
            <h2 class="flex items-center text-2xl font-bold text-[#1a1a1a] mb-10 tracking-tight uppercase">
                ...
                Browse Senior Care Listing by <span class="text-primary ml-2">TOP CITIES</span>
            </h2>

            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-x-8 gap-y-4">
                @foreach($cities as $city)
                    <a href="{{ route('listings.city', ['state' => $city['state_slug'], 'city' => $city['city_slug']]) }}"
        ...
        <div>
            <h2 class="flex items-center text-2xl font-bold text-[#1a1a1a] mb-10 tracking-tight uppercase">
                ...
                Browse Senior Care Listing by <span class="text-primary ml-2">State</span>
            </h2>

            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-x-8 gap-y-4">
                @foreach($states as $state)
                    <a href="{{ route('listings.state', ['state' => $state['slug']]) }}"
                        class="group flex items-center text-[13px] text-gray-500 hover:text-primary transition-colors duration-300">
